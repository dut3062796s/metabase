...
(defn find-and-load-all-drivers!
  ...
  (doseq [ns-symb @u/metabase-namespace-symbols
          ...
          :when   (not (#{:common :util :query-processor}
                        driver))]
    (try
...
(defn available-drivers []
  (set (for [driver (descendants driver/hierarchy ::driver)
  ...
  (sort
   (for [ns-symb     (clojure.tools.namespace.find/find-namespaces (clojure.java.classpath/system-classpath))
         ...
         :when       (and (or (starts-with? "metabase.driver")
                          ...
                          (find-ns ns-symb))
         [symb varr] (ns-publics ns-symb)
         :when       (instance? clojure.lang.MultiFn @varr)]
     (let [{:keys [arglists doc]} (meta varr)]
       (format "%s/%s %s" (ns-name ns-symb) symb arglists)))))
