...
(defn- require-driver-ns [driver & options]
  (let [expected-ns (symbol (str "metabase.driver." (munge (name driver))))]
    (println (trs "Loading driver {0} {1}" driver (apply list 'require expected-ns options))) ; NOCOMMIT
    (log/debug (trs "Loading driver {0} {1}" driver (apply list 'require expected-ns options)))
    (apply require expected-ns options)))
  ...
  (when-not (registered? driver)
    (du/profile (trs "Load driver {0}" driver)
     ...
     (when-not (registered? driver)
       ...
       (when-not (registered? driver)
         (throw (Exception. (str (tru "Invalid driver: {0}" driver)))))))))
...
(defn register!
  ...
  ;; next, if a parent driver was specified, load its namespace if not done already and make sure it is valid before
  ;; we alter the hierarchy in any way
  (when parent
  ...
  (doseq [parent (cons
                  (if abstract? ::driver ::concrete)
                  (cond
                    (coll? parent) parent
                    parent         [parent]))]
    (alter-var-root #'hierarchy derive driver parent))
...
(defn add-parent! [driver new-parent]
  (load-driver-namespace-if-needed driver)
  (register! driver :parent new-parent, :abstract? (abstract? driver)))

(defn dispatch-on-driver [x & _]
  (load-driver-namespace-if-needed x)
  x)
...
(defmulti available?
  "Is this driver available? (i.e. should we show it as an option when adding a new database?) This is `true` by default
  for all non-abstract driver types and false for abstract ones; some drivers might want to override this to return
  false even if the driver isn't abstract -- for example, the Oracle driver might return false if the JDBC driver it
  depends on is not available."
  {:arglists '([driver])}
  dispatch-on-driver
  :hierarchy #'hierarchy)
...
(defmethod supports? ::driver [_ _] false)
